...
1. **Full Refresh** (lento, ~2-3s):
   - Schermo lampeggia bianco→nero→immagine
...
2. **Partial Refresh** (veloce, ~300-500ms):
   ...
   - Movimento incompleto particelle → accumula ghosting
...
- **NON è burn-in permanente** (si risolve con full refresh)
...
- E-ink è PERFETTO per contenuti statici (eReader, dashboard, etichette)
- **Durata display**: migliaia di refresh, ma non milioni come LCD

### Controllo Software Refresh (M5GFX + EPDIY)

**Modalità disponibili** (epd_mode):
1. `epd_quality` (1): Full refresh, massima qualità (~15 frame, 2-3s)
...
- **Partial refresh**: possono essere più frequenti, ma → ghosting
...
// Gestione ghosting (responsabilità del dev!)
int partialCount = 0;
void smartRefresh() {
  if (partialCount >= 5) {
...
**Boot Behavior**:
1. **Power-on** (accensione) → Launcher si avvia (splash screen)
2. **Se nessun input** dopo splash → **autostart ultimo bin installato**
3. **Se premi bottone** durante splash → menu Launcher

**Workflow**:
```
Flash interna:           microSD:
├─ Bootloader           ├─ firmware1.bin
├─ Launcher (test)      ├─ firmware2.bin
└─ OTA partition ← bin installato  └─ MMpaper.bin

Boot → Launcher → [timeout] → bin installato
Boot → Launcher → [press btn] → menu
```

**Installare un .bin dalla SD**:
1. Launcher menu → naviga SD → seleziona .bin
2. "Install" → copia in OTA partition (flash interna)
3. Riavvio → autostart del bin installato

**Default bin all'avvio**: SÌ! Ultimo .bin installato si avvia automaticamente
...
### Auto-Update da GitHub Repository (AGGIORNA SD, NON FLASH!)

**Strategia implementata**:
- Launcher rimane l'UNICO firmware sulla flash (mai sovrascritto)
- MMpaper viene sempre caricato da Launcher dalla SD
- Auto-update SOSTITUISCE il .bin sulla microSD, NON la flash
- Dopo update: restart → Launcher carica nuova versione da SD

**Trigger aggiornamenti**:
- ✅ **Al boot**: Controlla SEMPRE al primo avvio
- ✅ **Schedulato**: Ogni giorno alle 2:00 AM (configurable)
- ✅ **NTP sync**: Sincronizza ora da internet al primo avvio
- ✅ **Timezone**: GMT+1 (Italia) con supporto ora legale

**Vantaggi**:
- ✅ Launcher mai toccato (sicurezza massima)
- ✅ Rollback facile (basta copiare vecchio .bin su SD)
- ✅ Nessun rischio di brick del device
- ✅ Launcher come "bootloader permanente"
- ✅ Aggiornamenti automatici senza intervento utente

**Funzionalità OTA esistente in Launcher**:
- ✅ Download da M5Burner repository (ufficiale)
- ✅ "Favorites" custom in `config.json`:
  ```json
  "favorite": [
    {
      "name": "MMpaper",
  ...
  ```
- ❌ **NON c'è auto-check al boot** (feature mancante!)

**La tua idea: Auto-sync GitHub → SD**
```
Boot → WiFi ON → Check GitHub releases → Download nuovi .bin → Avvia app
      ↓ No WiFi
      └─→ Skip check → Avvia app
```

**Status attuale Launcher**: check manuale (menu OTA → scegli → download)
...
**Opzione 2: Auto-update DENTRO MMpaper** (più semplice!)
```cpp
// In MMpaper stesso
void setup() {
  M5.begin();
...
- ✅ Non serve modificare Launcher
- ✅ Controllo totale su quando/come aggiornare
- ✅ Può scaricare altri .bin sulla SD (non solo se stesso)
- ✅ Logica custom (es: check solo se batteria >50%)
...
# MMpaper sul device
Boot → Check v1.0.1 > v1.0.0? → Download → Install → Restart
```

**Conclusione**:
- Launcher attuale = NO auto-check al boot
- **Meglio**: implementa auto-update dentro MMpaper usando esp_ghota
- Così MMpaper si aggiorna da solo + può gestire altri .bin custom!
...
- Tasto destro su M5Burner.app → "Mostra contenuto pacchetto"
...
**Community firmware**:
- M5Burner ha catalogo firmwares community (burn → estrai .bin da app package)
- M5Stack Firmware repo: lista di firmware esterni su GitHub

## Development Status
- ✅ Project initialized with PlatformIO
- ✅ M5Unified, EPDIY, esp_ghota libraries configured
- ✅ Auto-update da GitHub implementato in src/main.cpp
- ✅ Smart refresh management (partial/full) implementato
- ✅ Power management ottimizzato (IMU off, WiFi on-demand)
- ✅ Config file creato (include/config.h)
- ✅ README.md con istruzioni complete
- 🔄 Ready per prima compilazione quando arriva device
- ⚠️ Da configurare: WiFi credentials e GitHub user in config.h

## Features da Implementare in MMpaper

### 🔄 Auto-Update da GitHub (PRIORITÀ ALTA)
...
- Fallback: no WiFi/update → app avvia normalmente

**Workflow utente**:
```
Boot → [Background check GitHub] → Update? → Install → Restart
                                  ↓ No
                                  └→ Run app normalmente
```
...
void checkGitHubAndUpdate() {
  ...
  // WiFi connect
  // esp_ghota check
  // Se update: display progress → install → restart
  // Se no: WiFi disconnect → continua
}
```

**Benefits**:
- ✅ Update automatici senza computer
- ✅ Device sempre aggiornato in campo
- ✅ Rollout graduale: release → device si aggiornano
- ✅ Nessun intervento manuale utente
...
void setup() {
  auto cfg = M5.config();
  cfg.internal_imu = false;     // 👈 IMU OFF di default
  M5.begin(cfg);

  // WiFi OFF di default (già spento, non serve attivar nulla)
...
1. **WiFi**: solo quando serve (es: aggiornamento orario, fetch dati online)
   ...
   void updateOnlineData() {
     ...
     WiFi.mode(WIFI_OFF);  // 👈 Spegni dopo uso!
   }
   ```
...
2. **IMU/Gyroscope**: solo se triggato da azione utente
   ...
   void checkGesture() {
     ...
     M5.Imu.sleep();  // 👈 Torna in low power!
   }
   ```
...
3. **Timer/Schedule**: aggiornamenti periodici (es: ogni ora)
   ...
   void loop() {
     if (hourPassed) {
       updateOnlineData();  // WiFi on → fetch → WiFi off
       updateDisplay();     // Display on → refresh → Display off
       hourPassed = false;
     }
...
**Regola priorità**: Full refresh MAX ogni 10 secondi (limite hardware)
**Regola ghosting**: Full refresh ogni 5 partial (qualità immagine)

**Logica implementazione**:
```cpp
// Priorità: rispettare i 10s tra full refresh
// Se 5 partial avvengono in <10s → tollerare più partial
...
void smartRefresh() {
  ...
  if (needsGhostingFix && needs10sDelay) {
    ...
    partialCount++;
    // Se partialCount > 5 ma non sono passati 10s → accumula ghosting temporaneo
  }
}
```
...
```
**Vantaggio**: zero refresh inutili = batteria più duratura + display più longevo

## Power Management & Consumi Batteria

### Consumi misurati (PaperS3 @ 4.2V, batteria 1800mAh)
| Modalità | Consumo | Durata teorica | Note |
|----------|---------|----------------|------|
...
- **Impatto**: 9.28µA → 949.58µA se lasciato in full power
...
void setup() {
  ...
  // WiFi OFF di default (già spento, ma esplicito)
  // Se serve WiFi:
  // WiFi.mode(WIFI_STA);
...
// Durata batteria: 1800mAh / 72mAh = 25 giorni! 🎉
```
...
### Specifiche Display
- **Risoluzione**: 960×540 pixel @ 4.7" → **235 PPI**
...
**SÌ! I 16 livelli sono FONDAMENTALI per testo leggibile**
...
### Preview in VSCode
**NON c'è preview visivo diretto**, ma:
1. Calcola dimensioni: 960×540 @ 235 PPI ≈ 10.3×5.8 cm
2. Usa sketch cartaceo o tool online (es: Figma, Photoshop @ 235 DPI)
3. Test rapidi: emulatore web M5Stack (limitato)
4. **Meglio**: flash & test su device reale (e-ink è unico!)

### Font Size Recommendations (235 PPI)
| Size | Uso | Leggibilità |
|------|-----|-------------|
...
**Nota**: 235 PPI < 300 PPI (standard eReader) → usa font leggermente più grandi
